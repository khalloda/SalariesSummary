...
function parseSalariesSheet(worksheet: XLSX.WorkSheet): ParsedSalaryRecord[] {
  ...
  for (let i = headerRowIndex + 1; i < jsonData.length; i++) {
    ...
    // Skip empty, headers, placeholders, and total rows
    if (!name || 
        name === 'شركاء' || 
        name === 'م' || 
        name === '#' || 
        name === 'الاسماء / Name' || 
        name === 'Spare' || 
        name.includes('اجمالي') || // Skip total rows like "اجمالي الشركاء"
        name.match(/^\d+$/)) {
      continue;
    }
...
function parseAdditionsSheet(
  ...
  for (let i = headerRowIndex + 1; i < jsonData.length; i++) {
    ...
    if (!name || name === 'شركاء' || name === '#' || name === 'Spare' || name.includes('اجمالي')) {
      continue;
    }
...
function parseDeductionsSheet(
  ...
  for (let i = headerRowIndex + 1; i < jsonData.length; i++) {
    ...
    // Recalculate Net: Basic Salary + Yearly Increase + Bonuses - Deductions
